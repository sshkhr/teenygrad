...
In part two of the book, you will train a variety of deep neural networks with `torch`,
...
including 1. neural network primitives and optimizers, 2. automatic differentiation, and 3. gpu acceleration.
Because you have the necessary foundation in machine learning,
in Chapter [4. Learning *Sequences* from Data with Deep Neural Networks in `torch`](#4-learning-sequences-from-data-with-deep-neural-networks-in-torch)
...
and train the model using `torch`'s support for optimization and automatic differentiation with
...
After training a variety of deep neural networks with various inductive biases through `torch`,
in Chapter [5. Accelerating *Sequence Models* on `GPU` in `teenygrad` with `CUDA Rust` and `PTX`](#5-accelerating-sequence-models-on-gpu-in-teenygrad-with-cuda-rust)
...
and evolve `teenygrad`'s implementation for the third and final time to support a graph mode fusion compiler, and inference engine.

## II. Table of Contents

- [**4. Learning *Sequences* from Data with Deep Neural Networks in `torch`**](#4-learning-sequences-from-data-with-deep-neural-networks-in-torch)
  - [**4.1 From Linear to Non-Linear Learning**](#41-from-linear-to-non-linear-learning)
  - [4.2 Learning Embeddings with Feedforward Neural Networks]()
  - [**4.3 Learning Sequences with Feedforward Neural Networks**](#43-learning-sequences-with-feedforward-neural-networks)
  - [4.4 Learning Sequences with Convolutional Neural Networks]()
  - [4.5 Learning Sequences with Recurrent Neural Networks]()
  - [**4.6 Learning Sequences with Generative Pretrained Transformers**](#46-learning-sequences-with-generative-pretrained-transformers)
  - [4.7 Learning Some Deep Learning Theory]()
- [**5. Accelerating *Sequence Models* on `GPU` in `teenygrad` with `CUDA Rust` and `PTX`**](#5-accelerating-sequence-models-on-gpu-in-teenygrad-with-cuda-rust)
  - [**5.1 From Numerical Linear Algebra Libraries to Deep Learning Frameworks**](#51-from-numerical-linear-algebra-to-deep-learning-frameworks) <!-- `torch.nn`, `torch.optim` and `Tensor.backward()` -->
  - [5.2 Network Primitives and Optimizers with `teenygrad.nn` and `teenygrad.optim`]()
  - [5.2 Automatic Differentiation with `Tensor.forward()` and `Tensor.backward()`]()
  - [**5.3 Accelerating `GEMM` on Many Core GPUs**](#53-accelerating-gemm-on-many-core-gpus)
  - [5.4 Accelerating `GEMM` on GPU with Data Reuse]()
  - [5.5 Accelerating `GEMM` on GPU with Scheduling]()
  - [5.6 Accelerating `GEMM` on GPU with Tensor Cores]()
...
### 4.1 From Linear to Non-Linear Learning
...
### 4.6 Learning Sequences with Generative Pretrained Transformers
...
### 5.3 Accelerating `GEMM` on Many Core GPUs
