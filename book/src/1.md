...
## I. Table of Contents
- [1. Representing *Data* with High Dimensional Stochasticity in `torch` and `teenygrad`]()
  - <span style="white-space:nowrap">[1.1 From Software 1.0 to 2.0: Propositionally Deductive ELIZA to Probabilistically Inductive ChatGPT](#11-from-logically-deductive-software-10-of-eliza-to-probabilistically-inductive-software-20-of-chatgpt)</span>
  - [1.2 Distributing Data Stochastically with Probabilities of a Probability Space]()
  ...
  - <span style="white-space:nowrap">[1.5 Representing Data Dimensionally with Coordinates and Linear Transforms of a Vector Space]()</span>
  - [1.6 The Geometry of Coordinates via Inner Products: Lengths, Distances, Angles, Projections]()
  - [1.7 Matrix Factorizations: Eigenvalue and Singular Values Decomposition]()
  - [1.8 High Dimensional Stochastic Data = Random Vectors and their Distributions]() <!-- this is the goal for part 2 -->
  - [1.9 Dimensionality Reduction with Subspace Learning of Principal Component Analysis](#19-dimensionality-reduction-with-subspace-learning-of-principal-component-analysis)
- [2. Learning *Functions* from *Data* with Parameter Estimation in `torch` and `teenygrad`](#2-learning-functions-from-data-with-optimization-in-torch)
...
- [3. Accelerating *Functions* and *Data* with Basic Linear Algebra Subroutines in `teenygrad`](#3-accelerating-functions-and-data-with-basic-linear-algebra-subroutines-in-teenygrad)
  ...
  - [3.4 Accelerating Basic Linear Algebra Subroutines in Rust: Pipelines and Hierarchies]()
...
### 1.1 From Logically Deductive Software 1.0 of ELIZA to Probabilistically Inductive Software 2.0 of ChatGPT
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>

<!-- (embed nanochat example trained and inferenced by teenygrad). -->
...
(historical, philosophical introduction.. eliza and nanochat demos..)
...
To prepare you for such *representation learning* in [Part 2](), in [Chapter 1]() you will
1. construct the mathematical machinery to *represent such high dimensional stochastic data*
2. reproducing Tensorflow's Projector below by programming a*learning algorithm for dimensionality reduction*

In the first three subchapters proper (notwithstanding the current chapter) we place our focus
on probability theory, which is used to represent uncertainty around non-deterministic — *stochastic* — data:
  - [1.2 Distributing Word Data Stochastically with Probabilities of a Probability Space]()
  - [1.3 The Algebra of Probabilities via Rules: Sum Rule, Product Rule, and Bayes Rule]()
  - [1.4 Random Variables and their Distributions, Expectations, and Variance]()

thereafter, the next three chapters introduces the generalization of Cartesian algebraic geometry referred to as linear algebra,
which is used to represent data in *higher dimensions*:
  - [1.5 Representing Word Data Dimensionally with Coordinates of a Vector Space](#15-representing-word-data-dimensionally-with-coordinates-of-a-vector-space)
  - [1.6 The Geometry of Coordinates via Inner Products: Norms, Distances, Projections]() <!-- angles? rotation? orthogonality? (projection) -->
  - [1.7 The Fundamental Matrix Decomposition: Singular Value Decomposition]()

finally, combining the languages of probability theory and linear algebra let's us carry out the two tasks
  - [1.8 High Dimensional Stochastic Data = Random Vectors and their Distributions]() <!-- this is the goal for part 2 -->
  - [1.9 Learning Dimensionality Reduction with Principal Component Analysis]()
...
### 1.5 Representing Word Data *Dimensionally* with Coordinates of a Vector Space
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>

In the last three chapters you've started the transition from deductive software 1.0 to inductive software 2.0
by distributing truth stochastically with the language of probability theory.
In [Chapter 1.2]() you (todo). In [Chapter 1.3]() you (todo). In [Chapter 1.4]() you (todo).


But in order to do that, we must introduce the wonderful language of linear algebra — a generalization of linear transformations on the Cartesian coordinate system
which will let us represent words as random vectors in a multidimensional space $\reals^n$.
Take a look at the visualization below which contains 10,000 word vectors $\mathbf{x}\in\reals^{200}$ learned by the word2vec
model<span class="sidenote-number"></span><span class="sidenote">*Which we will train neural networks to learn for ourselves in [Part II]() following [Efficient Estimation of Word Representations in
Vector Space]() and [Distributed Representations of Words and Phrases
and their Compositionality]()*</span>
projected onto $\reals^3$.

<!-- Once we become comfortable with the basics of the high dimensional coordinates and linear transformations,
we can combine the two languages of probability theory and linear algebra together in the final
[Chapter 1.7 Probability + Linear Algebra = *High Dimensional Stochastic Distributions*]() -->

history, leading to the abstraction of a vector space.

Recall that in mathematics, most areas deal with various types of spaces and their objects,
as well as functions defined between such spaces and whether they preserve key
properties<span class="sidenote-number"></span><span class="sidenote">
*As we've just learned,
in [Chapter 1.2 Distributing Truth *Stochastically*](), the Kolmogorov axiomatization models probability spaces
with probability measures $\mathbb{P}\colon\mathcal{F}\to[0,1]$ defined on the set of all sample space $\Omega$ subsets referred to as the event space $\mathcal{F}$
which is required to to preserve the properties of (1), (2) and (3).*
</span>.
Specifically for linear algebra, the Peano axiomatization models vector spaces $\mathcal{V}$ with linear transformations which
need to preserve (1) vector addition $+\colon \mathcal{V} \to \mathcal{V}$ and
(2) scalar multiplication $\cdot \colon \mathcal{V} \to \mathcal{V}$.
Since all linear tranformations can be encoded as matrices, linear algebra is essentially the study of
vectors $v \in \mathcal{V}$ and matrices $A\colon \mathcal{V}\to\mathcal{V}$.

Once we've formulated the axiomatization of the abstract vector space for a definitional understanding of the mathematical objects
of vector and matrices, we will leave the ambience space behind in favor of computational programs which evaluate
such vectors and matrices by equipping the specific vector space $\reals^d$ with an inner product which in turn induces a norm, length, and
angle<span class="sidenote-number"></span><span class="sidenote">
*No different to how we left the definitional abstract probability space in favor of computational programs which evaluated probabilities with random variables $X$ and their distributions $p(X=x)$,
we will do the same with linear algebra.*
</span>
<span class="sidenote-number"></span><span class="sidenote">
*Defining mathematical objects definitionally rather than computationally has value besides posturing with formalization.
Machine learning's primary method in learning functions from data as we'll see in [Chapter 2. Learning *Functions* from *Data* with Optimization in `torch`]() is gradient descent.
And different variants of gradient descent exist by redefining and changing the notion of distance between vectors.*
</span>.

(axiomatize.defns)
- vector space, scalar multiplication, vector addition

$$
\begin{aligned}
\cdot \colon \mathbb{R} \times \mathcal{V} \to \mathcal{V} \\
(\alpha, \mathbf{v}) \mapsto \alpha \mathbf{v} \\
...
2\mathbf{v} &= \begin{bmatrix} 2 \\ 4 \\ 6 \end{bmatrix} \\

\mathbf{v} + \mathbf{w} &= \begin{bmatrix} 5 \\ 7 \\ 9 \end{bmatrix} \\

0.3\mathbf{v} + 0.7\mathbf{w} &= \begin{bmatrix} 3.1 \\ 4.1 \\ 5.1 \end{bmatrix}
\end{aligned}
$$

```python
{{#include ../../examples/1.5-tensor.py:1:7}}
```

- independance, basis, dimension
...
$$
AB = \begin{bmatrix} - & \mathbf{a}_1^\top & - \\ & \vdots & \\ - & \mathbf{a}_m^\top & - \end{bmatrix} B = \begin{bmatrix} - & \mathbf{a}_1^\top B & - \\ & \vdots & \\ - & \mathbf{a}_m^\top B & - \end{bmatrix}
$$

$$
\begin{aligned}
A\mathbf{x} &= \begin{bmatrix} 1 & 2 \\ 3 & 4 \end{bmatrix} \begin{bmatrix} 1 \\ 2 \end{bmatrix} \\
...
```python
{{#include ../../examples/1.5-tensor.py:8:14}}
```

splat teenygrad after math, torch, and teenygrad

### 1.6 The Geometry of Coordinates via Inner Products: Length, Distance, Angle, Projection
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>

```python
{{#include ../../examples/1.6-geometry.py}}
```

### 1.7 Matrix Factorizations: Eigenvalues, Singular Value, and their Decompositions
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>

```python
{{#include ../../examples/1.7-decompositions.py}}
```

### 1.8 High Dimensional Stochastic Data = Random Vectors and their Distributions
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>

### 1.9 Dimensionality Reduction with Subspace Learning of Principal Component Analysis
<small>[$\hookleftarrow$ Table of Contents](#i-table-of-contents)</small>

```python
{{#include ../../examples/1.9-dimreduce.py}}
```
...
"matrices" and we saw linear transforms in 2.1
